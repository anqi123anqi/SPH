    ...
    <div class="main">
      <div class="py-container">
        ...
        <div class="bread">
          ...
          <ul class="fl sui-tag">
            ...
            <li class="with-x" v-for="(attr,index) in searchParams.props" :key="index">
              {{attr.split(':')[2]}}<i @click="removeattr(index)">x</i>
            </li>
          </ul>
        </div>
        <!--selector-->
        ...
        <div class="details clearfix">
            ...
            <div class="navbar-inner filter">
              ...
              <ul class="sui-nav">
                <li :class="{active:isOne}" @click="changeOrder('1')">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{'icon-direction-down':isAsc,'icon-direction-up':isDesc}"></span></a>
                </li>
                <li :class="{active:isTwo}" @click="changeOrder('2')">
                  <a>价格<span v-show="isTwo" class="iconfont" :class="{'icon-direction-down':isAsc,'icon-direction-up':isDesc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
          ...
          <!--分页器 -->
          <MyPagination :pageNo="searchParams.pageNo" :pageSize="searchParams.pageSize" :total="total" :continue="5" @getpageno="getpageno" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    //删除分类的名字
    removeCategory () {
      //带给服务器的可无可有数据：如果属性时为空，空字符也会带给服务器，如果是undefined，当前属性不会带给服务器
      this.searchParams.categoryName = undefined;
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      this.getData();
      //严谨：本意是删除query，如果历经中出现params应保留，路由跳转应携带params参数
  ...
  watch: {
    ...
    $route () {
      //每一次请求完毕，应该把相应的category1Id，category2Id，category3Id的数据清空
      this.searchParams.category1Id = undefined;
      this.searchParams.category2Id = undefined;
      this.searchParams.category3Id = undefined;
      //再一次发起请求之前，整理参数
